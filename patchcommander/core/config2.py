    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="input-container"):
            if isinstance(self.value, bool):
                ...
                yield Static(f"Unsupported type: {type(self.value).__name__}")
...
class ConfigUI(App):
    ...
    def action_reset_all(self) -> None:
        ...
        for key in config.data.keys():
            widget = self.query_one(f"#reset_{key}", Button)
            if widget:
                widget.press()
    ...
    def action_quit(self) -> None:
        ...
        if self.changes:
            self.notify("You have unsaved changes! Press Save before quitting.", severity="warning")
            return
